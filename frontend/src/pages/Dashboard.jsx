...
const Dashboard = ({ activeUser, onTabChange }) => {
    ...
    const loadDashboardData = async () => {
      ...
      try {
        ...
        notes.forEach(note => {
          combined.push({
            type: 'note',
            title: `Uploaded note: "${note.fileName}"`,
            time: new Date(note.uploadDate),
            subject: note.subject,
            details: `Ready for AI summary generation`
          });
        });
  ...
  // Recharts Chart Data A: Weekly Study Hours
  ...
  // Recharts Chart Data B: Subject Mastery Levels
  const subjectMasteryData = [
    { name: 'Math', level: 82, color: '#6366f1' },
    { name: 'Physics', level: 90, color: '#8b5cf6' },
    { name: 'Chemistry', level: 75, color: '#ec4899' },
    { name: 'CompSci', level: 95, color: '#06b6d4' },
  ];
  ...
  const [quote] = useState(() => studyQuotes[Math.floor(Math.random() * studyQuotes.length)]);
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-12">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-white/5 dark:border-white/5 light:border-zinc-200/60 pb-6">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-white dark:text-white light:text-indigo-950 flex items-center gap-2">
            Welcome back, {activeUser.displayName || 'Student'} 👋
          </h1>
      ...
      {/* ==========================================
          2. METRIC STATISTICS GRID
          ========================================== */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        
        {/* STAT 1: NOTES UPLOADED */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* CHART A: STUDY HOURS WEEKLY TREND */}
        <GlassCard className="lg:col-span-2 p-5 border border-white/5">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-base font-bold text-white dark:text-white light:text-indigo-950">Study Progress Hours Trend</h3>
              <p className="text-xs text-slate-500 pl-0.5">Hours logged per day this week</p>
            </div>
          ...
          <div className="h-64 w-full text-xs">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={weeklyStudyData} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                ...
                />
                <Area type="monotone" dataKey="hours" stroke="#8b5cf6" strokeWidth={2.5} fillOpacity={1} fill="url(#hoursGlow)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </GlassCard>

        {/* CHART B: SUBJECT MASTERY LEVELS */}
        <GlassCard className="p-5 border border-white/5 flex flex-col justify-between">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-base font-bold text-white dark:text-white light:text-indigo-950">Subject Mastery Rating</h3>
              <p className="text-xs text-slate-500 pl-0.5">Diagnosed by quiz grades</p>
            </div>
          ...
          <div className="h-48 w-full text-xs">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={subjectMasteryData} margin={{ top: 5, right: 5, left: -30, bottom: 5 }}>
                ...
                />
                <Bar dataKey="level" radius={[8, 8, 0, 0]} barSize={28}>
          ...
          <div className="mt-4 border-t border-white/5 dark:border-white/5 light:border-zinc-200 pt-3 flex justify-between text-xs">
            <div className="flex flex-col items-center">
              ...
              <span className="text-[10px] text-emerald-400">95% (Peak)</span>
            </div>
            ...
            <div className="flex flex-col items-center">
              ...
              <span className="text-[10px] text-pink-400">75% (Needs work)</span>
            </div>
          </div>
        </GlassCard>
      ...
      <GlassCard className="p-6 border border-white/5">
        <div className="flex items-center justify-between mb-6 pb-2 border-b border-white/5 dark:border-white/5 light:border-zinc-200/50">
          <h3 className="text-base font-bold text-white dark:text-white light:text-indigo-950">Recent Study Activities</h3>
        ...
        {activities.length === 0 ? (
          <div className="text-center py-10 flex flex-col items-center gap-2">
            ...
            <p className="text-xs text-slate-500 max-w-xs">Upload your first study document in the notes section to start generating summaries and quizzes!</p>
            <button
              onClick={() => onTabChange('notes')}
